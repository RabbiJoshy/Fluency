...
# pipeline/artist/util_1a_artist_config.py -> pipeline/ -> Fluency/
...
_ARTIST_MENU_LEGACY = {
    "wiktionary": "sense_menu.json",
    "spanishdict": "sense_menu_spanishdict.json",
}
_ARTIST_ASSIGNMENTS_LEGACY = {
    "wiktionary": "sense_assignments.json",
    "spanishdict": "sense_assignments_spanishdict.json",
}
...
def ensure_artist_sense_dirs(layers_dir):
    os.makedirs(os.path.join(layers_dir, "sense_menu"), exist_ok=True)
    os.makedirs(os.path.join(layers_dir, "sense_assignments"), exist_ok=True)
    os.makedirs(os.path.join(layers_dir, "sense_assignments_lemma"), exist_ok=True)


def artist_sense_menu_path(layers_dir, source="wiktionary", prefer_new=True):
...
def artist_sense_assignments_lemma_path(layers_dir, source="wiktionary", prefer_new=True):
    ensure_artist_sense_dirs(layers_dir)
    new_path = os.path.join(layers_dir, "sense_assignments_lemma", "%s.json" % source)
    if prefer_new or os.path.exists(new_path):
        return new_path
    legacy_name = "sense_assignments_lemma.json" if source == "wiktionary" else "sense_assignments_%s_lemma.json" % source
...
def load_dotenv_from_project_root():
    ...
    from this file's location: pipeline/artist/util_1a_artist_config.py -> artist/ -> pipeline/ -> Fluency/
    """
...
# ---------------------------------------------------------------------------
# Re-exported from pipeline/util_6a_method_priority.py so existing imports keep working.

import sys as _sys
import os as _os
_sys.path.insert(0, _os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))))
from util_6a_method_priority import (METHOD_PRIORITY, TRANSLATION_PRIORITY,  # noqa: E402, F401
