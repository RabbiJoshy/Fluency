...
STEP_VERSION_NOTES = {
    ...
    # Intentionally NOT bumped for the ?langFrom=es fix — bumping would
    # invalidate the entire cache (~3h of scraping work). Instead,
    # pipeline/tool_5c_invalidate_backwards_entries.py identifies and
    # deletes only the entries that went backwards (translations look
    # Spanish), and the next cache-builder run refetches just those
    # missing entries through the corrected URL.
}
