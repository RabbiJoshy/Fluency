    ...
    if (card.isMultiMeaning) {
        ...
        if (currentMeaning && currentMeaning.targetSentence) {
            ...
            const spotifyBtn = spotifyTrackId
                ? `<button type="button" class="spotify-btn link-btn" onclick="event.stopPropagation(); spotifyPlayTrack('${spotifyTrackId}', ${positionMs})" title="Play in Spotify" style="cursor:pointer; background:none; border:none; margin:0; padding:6px;">${spotifySvg}</button>`
