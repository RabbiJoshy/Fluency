...
- English-first verb morphology is a persistent coupled subject + tense/mood cue, with the full analysis still available from the POS pill. One sense-linked Spanish example becomes a visible exact-answer cloze on first presentation and remains fixed for the attempt; the full multi-sense English fingerprint stays intact, back-side browsing cannot rewrite the completed prompt, and merged/restored mismatches abstain. Recognition sense groups default to all-open only when their measured rendered height fits, and manual expansion always wins thereafter.
