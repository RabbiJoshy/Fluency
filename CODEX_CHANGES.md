...
- The card-back unit keyed by `(lemma, POS)` is called a **lemma–POS group**. Selecting it changes the asserted lemma, current sense/subsense, example pool, POS styling, and visual order together; the chosen group/sense is pinned first without mutating stable card or sense indices.
...
- English-first verb morphology is a persistent coupled subject + tense/mood cue, with the full analysis still available from the POS pill. The full multi-sense English fingerprint stays intact; a generic Sentence hint reveals one sense-linked exact-answer Spanish cloze that was fixed when the attempt began, so back-side browsing cannot rewrite it and merged/restored mismatches abstain. Recognition sense groups default to all-open only when their measured rendered height fits, and manual expansion always wins thereafter.
...
- The earlier optional phrase-sidecar proposal is rejected for ordinary word cards: changing a whole-phrase English cue while asking for only one Spanish token creates an incoherent example-dependent prompt. The visible fixed Spanish cloze now supplies construction context while the main English fingerprint remains stable. Dedicated whole-construction cards may still be designed separately. Blank dictionary translations such as the `venir` reading of `vino` also cannot produce `came`; context-dependent imperfect and subjunctive readings intentionally abstain.
...
- Josh's proposed WSD framing for SpanishDict usage metadata is: let it behave *MWE-like as evidence* while retaining the identity/functionality of an ordinary sense. The source field is broader than literal `used with` (it includes `often` / `frequently`, alternatives, grammatical complements, constructions, and other context notes), so never turn it wholesale into a required MWE. Model it as a **sense-scoped construction/collocation feature**: a matched companion is positive evidence, absence is only weak negative evidence unless the source describes a required complement, and a visible active `used with` tag should require credible attachment rather than merely echoing metadata beside an example where the companion is absent.
- Same-sentence presence is not enough to claim that usage evidence applies. Confidence should rise from sentence co-occurrence → clause/proximity → dependency/construction attachment; only the last tier should support a confident WSD match, while weaker tiers remain labelled possible textual matches. Targeted harvesting may prefer sentences containing the combination because they are pedagogically useful, but that selected sample must remain separate from the unbiased corpus sample used to estimate sense prominence or it will bias the very frequency being measured. Evaluate baseline vs companion vs clause/dependency features on the acceptable-sense benchmark, not exact-leaf agreement. The production WSD integration stays paused until the rewrite can represent those tiers; the card's Dictionary presentation pass is the inspection tool first.
- Other promising SpanishDict features currently absent from the embedding gloss include sense regions/register, exact headword/lemma identity, redirect/conjugation relation, the second dictionary example, stable sense order/IDs, and sense-aligned thesaurus links. Preserve and ablate them as explicit priors/features rather than silently folding them into observed corpus frequency.
...
- Implementation commit `6f85b7a4`; front-end cache `flashcards-v264` / `20260822d`.
- Upgraded `FlaggedWords` from schema v2 to v3 with non-destructive v1/v2 migration and backup. New flags retain the exact run/release fields available at review time plus a bounded JSON provenance snapshot; older or partially attributed decks degrade to blank optional identity fields rather than failing.
- Upgraded the existing `SongSets` contract from v1 to v2 so one per-user row stores both exact song IDs and their contributing artist slugs. Existing rows remain valid and gain the new column in place.
- Added `JSTA` to the centralized audit capability used by information, provenance, report, and flag controls. User progress remains separated by the unchanged initials-based persistence key.
- Verification: Apps Script migration/round-trip test, focused audit and SongSets tests, JavaScript syntax, Python compilation/JSON validation, asset-version lockstep, and `git diff --check`. The full Node suite remains 72/80 with the eight unrelated baseline failures recorded above.
...
### 2026-08-19 — Show a fixed Spanish cloze on English-first cards

- Implementation commit `6aff4252`; front-end cache `flashcards-v262` / `20260819a`.
- Preserved the bounded multi-sense English fingerprint and made its existing sense-linked Spanish sentence cloze visible immediately. The blank remains keyed to the complete authoritative production answer, including expression and clitic forms; exact-answer mismatches still abstain.
- Captured the cloze once per card attempt in render-only weak state. Cycling examples or senses after revealing the back cannot retroactively change the prompt already answered; entering the card again may select a new example.
- Rejected the earlier phrase-sidecar design for ordinary cards and left its isolated experimental branch unmerged. This app-only change does not read or modify WSD, assignments, deck data, IDs, frequencies or progress.
- Verification: fourteen focused reverse-cue/presentation tests, JavaScript syntax, 74 asset tags in lockstep, and `git diff --check`. The full Node suite is 69/77 with eight unrelated baseline failures: one personalised-frame fixture, one offline checksum, one Spanish song-catalog fixture, and five stale UI assertions.
