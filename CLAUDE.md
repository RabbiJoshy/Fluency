...
| Builder filters | `--remainders` (SENSE_CYCLE buckets, default off) and `--min-priority N` (drop low-priority claims) on both `step_8a` and `step_8b`. `--min-priority` default is language-specific: `config/config.json` → `languages.<lang>.pipelineDefaults.minPriority` (Spanish: 50; unset languages: 0). Resolved by `pipeline/util_pipeline_config.py`. Orthogonal flags: combine to get sparsest/full/clean/catch-all decks. |
...
- **Builder toggles**: `step_8a` (normal) and `step_8b` (artist) share two orthogonal flags: `--remainders` (default off, emits SENSE_CYCLE buckets for orphan examples) and `--min-priority N` (drops assignments below priority N; auto-assignments always pass through). `--min-priority` default is language-specific, sourced from `config/config.json` → `languages.<lang>.pipelineDefaults.minPriority` (Spanish opts in to 50; unset languages fall back to 0). `step_8a` hardcodes `language="spanish"`; `step_8b` reads `artist.json.language`. Resolver: `pipeline/util_pipeline_config.py`.
