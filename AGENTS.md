...
`config/dev_changelog.json`, `CODEX_CHANGES.md`) and the cache-version bumps;
Claude owns the **pipeline/data engine** (`pipeline/`, `Data/`, `Artists/`, `docs/`).
Stay on Codex's side unless a cross-boundary task is explicitly agreed.

**GitHub Issues is the live backlog.** Read the backlog rules in `COLLABORATION.md`. Search
`https://github.com/RabbiJoshy/Fluency/issues` before creating work, update an overlapping issue
instead of duplicating it, and use the repository's owner/horizon/size plus applicable
mode/language/surface/topic labels. `TODO.md` and `TODO_PIPELINE.md` are historical snapshots only.
...
`CODEX_CHANGES.md` is a cross-model handoff, not a replacement for git, GitHub Issues, or the developer changelog.
